...
1. 诺艾尔的血条变成"**次数血**"：血量上限 = `floor(当前最大生命 / 45)`；
2. 掉血后 **2 秒内**血量不会再次降低。
...
| 环节 | 做法 |
|---|---|
| 上限换算 | `maxhp` 字段直接换成次数：`hitMax = max(1, floor(真实最大生命 / 45))`（150 → 3 次，200 → 4 次）。AIC 血条是比例驱动的，所以自动显示成"3 格血" |
...
1. 诺艾尔装上坚硬外壳 → 血条变成 `floor(最大生命 / 45)` 格（150 → 3）；坐着/起身不影响；
2. 挨一下：掉 1 格；**2 秒内**再挨：血量不动（受击演出照旧）；
3. 超过 2 秒再挨：再掉 1 格；掉到 0 → 正常战败与长椅复活，复活后回满；
4. 卸下护符：血条回到正常数值并按剩余次数折算（例：剩 2/3 → 100/150）；
5. 带着护符存档 → 读档：仍是次数血，真实上限不丢（`kic_noel_sturdy_maxhp` = 150）。

验证：`build=2026-09-22.11`，DLL SHA256 `D80DE51015DD42CC…`（9,036,800 B，两份 0.30g 安装已同步；只覆盖 DLL，未动素材）。
