...
### 37.7 【2026-09-23】小剑山渲染缩小为 50%

需求：小剑山的渲染大小缩到现在的 50%。

- 代码默认：`SpikeScale` 0.22 → **0.11**；
- 同样按 36.7 的教训，把单机安装 cfg 里已经写下的 `SpikeScale = 0.22` 手改成 **0.11**
  （联机那份的 cfg 还没有 `[Charm25]` 段，重启后会按新默认值生成）。

验证：`build=2026-09-23.29`，DLL SHA256 `19F80DB7AA65BB73…`（两份安装已同步；只覆盖 DLL，cfg 已同步单机那份）。

---

## 38. 护符 24 防御者纹章（诺艾尔侧，2026-09-23，build=2026-09-23.30）

**需求**：效果与小骑士的完全相同。

小骑士那套是"**蓝色魔力法阵**"（`KnightEntity.cs:2923-3141` + 三层渲染），诺艾尔这边按同一套参数移植：

| 组成 | 参数（与小骑士一致） | 挂载层 |
|---|---|---|
| 实心圆 | 半径 **3 格**，深蓝（0.1,0.18,0.75），浓度由程序化径向贴图控制 | `PR0`（身后） |
| 圆内伤害 | 进入立刻 **10** 点；之后**每 1 秒**仍在圆内再 10 点（`fix_damage`，`huttobi_ratio=-100` 压制击飞 → 只保留受击硬直，可击晕） | — |
| 陷阱清除 | 圆内摧毁蜘蛛陷阱/蛛丝球（扫 `MGContainer` 里 `MGKIND.BASIC_SHOT` 且 `Other is MgBsSpiderTrap.MEM` 的实例并 `kill(0f)`） | — |
| 扩张圆环 | 每 **3 秒**从中心发射，**12 格/秒**扩张，到边缘发光 0.25s 并依次出现 星→方→叉 图案（满亮度 0.3s → 淡出 0.3s） | `PR2`（动态层） |
| 公转球体 | **5 颗**深蓝球，球心距 **1.75 格**、半径 0.5 格、72° 均布、**2.5 秒**一周 | `PR1`（身前） |

- 贴图全部**复用骑士侧的程序化生成方法**（`KnightEntity.MakeShelterCircleTexture(128)` /
  `MakeShelterSphereTexture(64)`，`internal static` 同程序集可直接调），因此**不需要任何素材部署**；
- 图案绘制（星/方/叉）把骑士的 `DrawShelterPattern` 原样搬成 `DrawNoelShelterPattern`；
- 陷阱去重集合按实例记，超过 64 个时清空（魔法实例会被池复用）。

### 38.1 配置

| 分组 | 键 | 默认 | 含义 |
|---|---|---|---|
| `Charm24` | `CircleRadius` | 3 | 法阵实心圆半径（格） |
| `Charm24` | `CircleDamage` | 10 | 法阵单次伤害 |
...
### 40.3 【2026-09-23】小剑山也不攻击魔物"剑山"

**需求**：诺艾尔发光子宫生成的小剑山**不会去攻击魔物剑山**。

在三处加了 `IsUniEnemy(enemy)` 跳过（剑山 **及其污染体**，同一判断）：

1. **索敌**（`FindNearestNoelSpikeTarget`）：不再把剑山当成目标 → 小剑山只会追别的魔物；
2. **触碰判定**（`NoelSpikeHitEnemy` 的小半径检测）：剑山不算"碰到了敌人" → 不会在它身上爆炸，
   小剑山**继续飞**（不会被白白消耗）；
...
## 42. 护符 27 深度聚集（诺艾尔侧，2026-09-23，build=2026-09-23.35）

**需求**：① 诺艾尔**无法使用法术**；② 长按法术键会走**咏唱魔法的动画**，但**不弹法术选择界面**；
③ 长按法术键 **0.5 秒**后开始**把 1 MP 转成 1 HP**，速度 **20 MP/秒**，直到松开法术键，
并在开始回血的同一刻**结束咏唱动画**。
...
| 效果 | 做法 | 出处 |
|---|---|---|
| ① 无法使用法术 | `M2PrSkill.explodeMagic` **前缀**：清蓄力（`killHoldMagic(false,…)`）后 `__result = false; return false;` → 走原版"这一发没成型"的既有分支，不产生任何子弹 | `M2PrSkill.cs:3611` |
| ② 咏唱动画照常、不弹选择界面 | 拦 `ActiveSelector.selectInit`（选择界面的**注册入口**）→ 不注册绘制、`slowInit()` 直接返回 false（不画选择环、也不进入选择慢动作），但咏唱流程本身不受影响 | `ActiveSelector.cs:85`，调用点 `MagicSelector.cs:1066` |
| ③ 长按 0.5s 后 MP→HP | 每帧 tick：`PR.isMagicO()`（原版咏唱同一个判据，`M2PrSkill.cs:3226`）为真则累加计时；满 `HealDelay`（0.5s）后按 `20 MP/秒` 扣 MP、**1 MP = 1 HP** 回血，并清掉蓄力（=结束咏唱动画） | 本模组 tick |

### 42.2 细节

- 咏唱动画前半段就是**原版流程**：按住法术键 → `MAGIC_CHANT_DELAY = 14 帧`（≈0.23s）后开始咏唱，
  界面被我们拦掉，所以只有动画；到 0.5s 我们接管（清蓄力 + 开始回血）。
- 回血用 `PR.applyMpDamage` 扣魔（法术扣魔同一条路，魔力条正常刷新），
  用 `hp` 字段直写 + `RefreshNoelHudHp()` 回血（与坚硬外壳/坚固心脏同一套读写方式）。
- MP 以整数结算（`20 MP/秒` → 每约 3 帧扣 1 点），小数部分累积、不丢；
- **HP 满了就不再消耗 MP**（避免白烧魔力）；松开法术键立刻停止并清零计时。
- 只对本地诺艾尔 + 诺艾尔模式 + 佩戴深度聚集生效。

### 42.3 配置

| 分组 | 键 | 默认 | 含义 |
|---|---|---|---|
| `Charm27` | `HealDelay` | 0.5 | 长按多久开始回血（秒） |
...
## 43. 护符 28 生命血之心 / 29 生命血核心（诺艾尔侧，2026-09-24，build=2026-09-24.3）

**需求**：28 → **HP 上限 -50、MP 上限 +50**；29 → **HP 上限 -100、MP 上限 +100**；HP 下限 1。

### 43.1 为什么和护符11 合成一套"上限修正"

护符 11 坚固心脏（+120 HP 上限）原本自己一套"基础上限 + 加成"的写回逻辑。
若 28/29 各自再写一次 `maxhp/maxmp`，三个护符会**互相覆盖**（每帧最后一个写入者生效）。
所以把它们统一成**一处计算**（`TickNoelHeartCharm`，行为不变、只是扩展）：
...
| 情况 | 结果 |
|---|---|
| 只戴 28 | HP 上限 = 基础-50（≥1）；MP 上限 = 基础+50 |
| 只戴 29 | HP 上限 = 基础-100（≥1）；MP 上限 = 基础+100 |
| 28+29 同戴 | HP 上限 = 基础-150（≥1）；MP 上限 = 基础+150 |
| 11+28 同戴 | HP 上限 = 基础+70 |
| 全卸下 | 还原为基础值，并把当前 HP/MP 钳回上限 |

### 43.2 细节

- 基础上限寄存在 COOK SF（`kic_noel_heart_base` / `kic_noel_heart_base_mp`，随存档序列化），
  读档回到"已佩戴"时据此重新激活，**不会重复叠加**；
- 首次佩戴坚固心脏时仍把新增的 +120 直接补成当前血量（保持原来的观感）；
- **MP 上限增加不会顺带补满当前 MP**（只在卸下/上限下降时把当前值钳回去）——
  如果你希望戴上时也把新增的 MP 直接补上，说一声即可；
- HP 上限下降时把当前 HP 钳到新上限（下限 1）；HUD 会立刻刷新。
- 数值是常量（同护符 11 的 +120），需要可调再说。

验证：`build=2026-09-24.3`，DLL SHA256 `D0BC96A970E1979F…`（两份安装已同步；只覆盖 DLL）。

### 43.3 【2026-09-24 数值调整】28 → HP-30，29 → HP-60

需求：**28 生命血之心 HP 上限 -30（MP 仍 +50）**；**29 生命血核心 HP 上限 -60（MP 仍 +100）**。

| 情况 | 调整后 |
|---|---|
| 只戴 28 | HP 上限 基础-30（≥1）；MP 上限 基础+50 |
| 只戴 29 | HP 上限 基础-60（≥1）；MP 上限 基础+100 |
| 28+29 同戴 | HP 上限 基础-90（≥1）；MP 上限 基础+150 |
...
### 44.3 【2026-09-24】效果1/3 定案 + 池空死亡

用户选择：**效果1 由模组自绘蓝色 HP 条**（纯色 `#0045FF`）；**效果3 直接不显示 HP 数字**；
**魔力池打空 = 诺艾尔死亡**。

| 效果 | 实现 |
|---|---|
| 1 HP 条染色 | 在 `KnightHudDeco.OnGUI`（诺艾尔模式且佩戴乔尼时）自绘一条纯蓝矩形：矩形直接取 `UIStatus.MdH`（HP 条填充网格）前 4 个顶点换算成屏幕像素，因此位置/宽度与原条**完全一致**，正好盖住原来的填充段；颜色 `#0045FF`（`(0, 69/255, 1)`） |
| 3 不显示 HP 数字 | HP 数字与 MP 数字共用网格 `MdGageT`，唯一区别是 HP 那次传了 `bounds_w = bounds_hp_w`（>0）、MP 那次传 0（`UIStatus.cs:2168` vs `:2194`）。于是拦 `BMListChars.DrawScaleStringTo`：**网格 == MdGageT 且 bounds_w > 0** 且佩戴乔尼 → 不画（MP 数字照旧） |
| 池空死亡 | 扣血改扣魔之后，若 `mp <= 0` → 置内部标记后调用 `applyHpDamage(9999, …)`（该标记让"扣血改扣魔"放行）→ 走 AIC 原版强制死亡流程 |

验证：`build=2026-09-24.6`，DLL SHA256 `41EB15EC93D41DEA…`（两份安装已同步；只覆盖 DLL）。

### 44.4 【2026-09-24 改用模组已有机制】HP 条染色走 `UIStatus.redrawAll` 顶点重染

**用户提示**："小骑士的血条是黑色的" —— 顺着这条线索查到：模组**早就有**染色机制，
就是骑士模式那条路：`CombatGuard.RedrawAllPostfix`（挂 `UIStatus.redrawAll` 后缀）
用 `RecolorMesh` 逐顶点改 `MdH` 网格颜色，注释里甚至已经写了
"血条颜色优先级：蓝色（生命血/乔尼的祝福）> 橙色（蜂巢之血）> 黑色（默认）"。

所以诺艾尔侧不必自绘矩形：**加一个 `UIStatus.redrawAll` 后缀**，佩戴乔尼时把
`MdH` 网格**前 4 个顶点（= 填充段）**染成 `#0045FF`（只染填充段、其余顶点不动，
空条外观与原版一致）。位置/长度仍由游戏自己算，不受分辨率影响。

（此前的 OnGUI 自绘矩形方案已整体删除；`KnightHudDeco` 里相关代码与诊断一并移除。）

验证：`build=2026-09-24.10`，DLL SHA256 `269582819FF4257D…`（两份安装已同步；只覆盖 DLL）。
...
### 50.4 【调整】精华逸散范围与黄色粒子一致（1~1.8 格）

反馈：精华飞得太远。改法：每个精华粒子生成时随机一个**最远距离**
`TargetDist = 1~1.8 格`（就是黄色粒子的**生成半径**那一套随机值，也就是黄色粒子的作用范围），
向外扩散到该距离即消散；透明度按"接近最远距离"淡出（最后 0.4 格），并保留 0.05 秒淡入。
这样两类粒子的空间范围完全一致，只是方向相反。

（速度仍是翻倍后的 6~8 格/秒、仍受 `ParticleSpeedScale` 影响 —— 1~1.8 格大约 0.15~0.3 秒走完，
所以精华看起来是"从中心快速逸散一小段后消失"。若想更慢，把 `ParticleSpeedScale` 调小即可。）
...
> 追加：**跳跃键也一起锁**（`KEY.SIMKEY.JUMP`）。现在蓄力期间的锁定集合 =
> 移动 L/R/T/B（含 LA/RA/TA/BA）+ 攻击 Z + 法术 X + 跳跃 JUMP；护盾键 LSH 仍然放行（松键要用来触发冲刺）。
> `build=2026-09-25.8`，DLL SHA256 `7F6D20F32845EBE1…`。

### 51.7 【修正】空中蓄力也要摆 chant（原来被 fall 动作占了）

**原因**：姿势改写只覆盖了待机族 `stand*`，而空中请求的是 **`jump` / `fall`**
（`AnimationShufflerNoel.cs:658`：上升中 `"jump"`、下落/滞空 `"fall"`），所以空中一直是下落动作。

**改法**：改写集合扩成 **待机 + 空中**：`stand*` / `jump*` / `fall*`（仍跳过带 `'2'` 的过渡姿势，
如 `stand2sink`、`fall2`）。蓄力中只要是这三个族的请求，一律换成本护符的咏唱姿势，
于是空中长按护盾键也会播 `chant`（配合前面加好的「猫之缓降」软着陆管线正好是"悬停吟唱"的观感）。

验证：`build=2026-09-25.9`，DLL SHA256 `9467115679277B8F…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 51.8 【修正】冲刺穿墙：改用游戏自己的物理推进（walkBy + checkwall）

**现象**：冲刺期间诺艾尔会直接穿墙。
**原因**：`.3` 起是用 `pr.setTo(...)` 每帧硬定位本体（相当于瞬移），完全绕过碰撞。

**改法**：位移交给 AIC 的物理管线 ——
`pr.walkBy(FOCTYPE.WALK, dir × DashBurstSpeed / 60f, 0f, checkwall: true)`：
`walkBy` 内部是 `Phy.addFoc(foctype | FOCTYPE._CHECK_WALL, dx, dy, …)`，
**带墙检测**，撞墙就会被挡下（原版闪避/滑铲都用它位移）。
（速度换算：AIC 的位移量是"格/帧@60"，所以 `格/秒 ÷ 60`。）

配套两点：
- **图片改为跟随本体的真实位置**：每帧 `图片X = pr.x + 朝向 × DashBurstBackOffset`
  （`图片Y = 身体中心`），于是她停下时图片也停，不会出现"人卡墙里、图片继续飞"；
- **撞墙提前收尾**：连续 3 帧水平位移 < 0.02 格就认为被墙挡住，直接结束冲刺段（同样走
  `EndNoelShadowDash`：白屏 + 还原本体），避免图片在墙上空转。

另外把"锁输入"的条件从"仅蓄力中"扩成"蓄力中 **或** 冲刺段进行中"，
免得玩家用方向键/跳跃干扰这段位移。
...
### 52.4 【修正】"法杖附了魔法霰弹"判据用错了

**现象**：附魔魔法霰弹后冲刺仍然只造成轻攻击伤害、也没有霰弹命中特效。

**原因**：判据写成了 `pr.isShotgunState()` —— 但它**不是**"法杖带霰弹附魔"，
而是"正在挥出去的霰弹/变种那几帧的状态"（`PR.cs:6012-6035`：
`WHEEL_SHOTGUN / COMET_SHOTGUN / DASHPUNCH_SHOTGUN / AIRPUNCH_SHOTGUN / EVADECOUNTER_SHOTGUN / UKEMI_SHOTGUN`）。
站着（准备冲刺）时恒为 false → 一直走轻攻击分支。

**改法**：判据换成**手里握着魔法蓄力**：`IsNoelMagicChanting(pr)`
（`CurMg != null && CurMg.isPreparingCircle && getHoldingMp(true) >= 1`；护符22 巴尔德之壳用的同一个），
这才是"法杖附了魔法霰弹"的状态。
同时把伤害分支改成"**附魔中就是霰弹结算**"：缓存只是数值来源，没有缓存时走
"按原版公式现算"，而不允许退回轻攻击；若连公式都算不出（蓄力已失效），
才用一个最小攻击包 + 真伤兜底，保证伤害落地。

验证：`build=2026-09-25.13`，DLL SHA256 `A5E9C9555B19E95A…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 52.5 【修正】只打到第一个目标就停下：撞墙误判

**现象**：冲刺命中第一个目标后诺艾尔立刻停下，后面的目标都吃不到伤害。

**原因**：打中敌人时 AIC 会进入**命中停滞**（`Map2d.TS → 0`，物理那几帧不推进），
而 51.8 加的"撞墙提前收尾"判定只看"这帧水平位移是否 ≈ 0" → 停滞的 3 帧就被误判成撞墙，
冲刺立刻结束（于是只结算了第一个目标）。

**改法**：
1. 停滞期间**跳过撞墙判定**（`Map2d.TS < 0.1` → 计数器清零），并把阈值放宽到 5 帧、epsilon 收紧到 0.01；
2. 冲刺两个阶段的时间推进改用**游戏时间**（`Time.deltaTime × clamp(Map2d.TS, 0, 1)`），
   于是命中停滞/慢动作期间冲刺计时也会暂停，不会平白吃掉冲刺时长（也不会把停滞误当成"没在动"）。

路径判定本身没变（每帧以身体中心 2×2 格 OverlapBox，`HashSet` 保证同一只只挨一次），
所以修好后沿途**所有**被扫到的目标都会各吃一次伤害。
...
### 54.4 【追加】效果3：蹲下 / 爬行时持续回血（5 HP/秒）

`TickUnnCrouchHeal(pr)`（挂进 `TickNoelCharmEffects`）：

- 条件 = `UnnFriendlyActive()`（佩戴34 + 蹲下/爬行）+ 存活 + HP 未满；
- `_unnCrouchHealAccum += 5 × Time.deltaTime`，累计到 1 点就 `pr.cureHp(1)`（小数不丢，
  不会因为帧率低而少回血）；回满即截断；
- 回血走 AIC 原生 `cureHp` → **佩戴乔尼的祝福时会被自动改写成回魔**（与护符31 蜂巢之血同一口径）；
- HP 已满 / 起身 / 卸下护符 → 累积量清零。

配置：`Charm34/CrouchHealPerSecond`（默认 5；0 = 不回血）。
...
### 55.3 【补充】突进冲击冲突 + 旋风斩期间禁用攻击键

**问题**：诺艾尔的「突进冲击」是**奔跑中按下攻击键**触发（`M2PrSkill.getPunchVariation`：
`Pr.run_continue_time >= 22f && isEnable(SKILL_TYPE.dashpunch)` → `STATE.DASHPUNCH`），
它会在按下那一帧就把状态改走，于是"长按攻击键蓄力"根本没机会开始。

**改法**：在 `M2PrSkill.isEnable(SKILL_TYPE)` 前缀（原本给护符33 用）里加一条 ——
**佩戴护符35 且攻击键正被按住**时，`dashpunch` 一律返回 false，
于是那一帧的变招回落成普通轻攻击 `PUNCH`，长按就能顺利进入蓄力。

> 副作用（需要你确认）：**奔跑中点按**攻击键时也不会再触发突进冲击，而是普通轻攻击。
> 因为"是否长按"在按下那一帧无法预知，要保留点按突进冲击就只能改成
> "按下时先压制 N 帧、若在窗口内松手再由模组补一次 DASHPUNCH"的写法 —— 需要的话告诉我。

**旋风斩期间禁用攻击键**：`EV.lockPrInputManipulate` 前缀里，旋风斩（含收尾）期间的
封锁集合从"L/R/T/B + JUMP"扩成"L/R/T/B + JUMP + **Z（攻击）**"。

验证：`build=2026-09-25.22`，DLL SHA256 `52DAF8B5B7BDE27C…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 55.4 【补充】空中蓄力时禁用凌空横斩（Airpunch）

与突进冲击同理：**空中按下攻击键**会变招成凌空横斩（`getPunchVariation` 空中分支：
`state == PUNCH && isEnable(SKILL_TYPE.airpunch)` → `AIRPUNCH`），会把长按蓄力吃掉。

改法：同一条 `M2PrSkill.isEnable` 前缀里，把护符35 的拦截面扩成 **`dashpunch` + `airpunch`**：
- `dashpunch`：佩戴35 且**攻击键被按住** → 拦（奔跑中的情况）；
- `airpunch`：佩戴35 且**攻击键被按住 + 当前不在地面（`!hasFoot()`）** → 拦。

同样地，空中**点按**攻击键也不会再触发凌空横斩（回落成普通轻攻击）——原因与上面一样：
按下那一帧无法预知是否会长按。要保留点按行为就得上"延迟 + 松手时补发"的方案。

验证：`build=2026-09-25.23`，DLL SHA256 `97956BA0C1D9E931…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 55.5 【补充】点按要保留原招式：松手时"补发"凌空横斩 / 突进冲击

需求补充：**空中点按仍要能触发凌空横斩**（推而广之，奔跑中点按也应保留突进冲击）。

做法（`TickNailMasterTap`，挂在护符35 的每帧 tick 里）：

1. 检测攻击键的**按下 / 松开**边沿；
2. 按下时若"在空中"或"奔跑中"→ 开一个点按窗口，开始计时；
3. 松手时若计时 ≤ `Charm35/TapSeconds`（默认 **0.18 秒**）→ 判定为**点按**，
   由模组按原版规则补发一次：
   - 空中 → `pr.changeState(AIRPUNCH_SHOTGUN / AIRPUNCH)`
   - 奔跑中 → `pr.changeState(DASHPUNCH_SHOTGUN / DASHPUNCH)`
   （与 `getPunchVariation` 里 `CurMg != null ? *_SHOTGUN : *` 完全一致；
   原版状态机自己会结算动画与伤害，所以不需要我们重做那套）；
4. 超过窗口 → 视为长按，交给蓄力，不补发。

> 已知的小瑕疵：按下那一帧原本会走一次普通轻攻击（因为变招被我们压掉、回落成 `PUNCH`），
> 点按补发会在松手帧切到凌空横斩，所以点按的头几帧可能看到一点点轻攻击起手。
> 若观感不能接受，可以改成"按下时把整个 punch 管线压 N 帧"（代价是会连带压掉长按时的其它攻击派生）。

验证：`build=2026-09-25.24`，DLL SHA256 `F14CF739E037C57D…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 55.6 【数值】佩戴骨钉大师的荣耀时，诺艾尔造成伤害 ×5

需求：佩戴护符35 时诺艾尔造成的伤害提升 **5 倍**（该护符已锁魔法键，此时她的伤害都是无附魔的）。

落点：**统一乘区** `NoelFinalDamageMult(kind, shotgunFlavored)` 里最前面加一项
`if (佩戴35) mult *= NailMasterDamageMult(默认5)`；同时把 `ShamanCircleCastPrefix` 的
早退条件（原来只放行 5/13/16/27）补上 **35**，否则只戴35 时那个前缀会直接 return、×5 不生效。

因为诺艾尔的近战/技艺/法术伤害最终都经过 `MGContainer.CircleCast`（护符13 坚固力量就是靠它生效的），
而蜕变挽歌剑气与护符33 冲刺伤害也共用同一个 `NoelFinalDamageMult`，
所以这一条是"她造成的**所有**伤害 ×5"，并且与萨满之石/坚固力量/会心/深聚是**连乘**关系。

配置：`Charm35/DamageMult`（默认 5）。
...
### 56.4 【补充】空中蓄力时同时禁用"旋风斩击"（Wheel），点按补发也按原版规则给 WHEEL

空中"攻击键 + 左右方向"会变招成**旋风斩击**（`getPunchVariation` 空中分支：
`(isLO(4)||isRO(4)) && canStand(...) && isEnable(SKILL_TYPE.wheel)` → `WHEEL`），
与空中长按蓄力冲突，所以把护符35 的压制名单从 `dashpunch/airpunch` 扩成
**`dashpunch` / `airpunch` / `wheel`**（条件仍是"佩戴35 + 攻击键被按住"，其中空中两招额外要求 `!hasFoot()`）。

同时把"点按补发"按原版判定顺序补全：空中点按松手时，若**按着左右方向且脚下有落脚点**
（`canStand(x, mbottom + 0.12)`）→ 补发 `WHEEL_SHOTGUN / WHEEL`（即旋风斩击），否则才补发凌空横斩。

> 同源的"彗星俯冲"（空中 + 下方向 + 攻击键，`comet`）也是同一形态；需求没点名，暂时没动，
> 要一起禁就说一声。

验证：`build=2026-09-26.3`，DLL SHA256 `BE7AF85F80B554CD…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 56.5 【补充】彗星俯冲也一起禁用

护符35 的压制名单再加 `comet`（彗星俯冲 = 空中 + 下方向 + 攻击键，
`isBO(4) && canStand(...) && isEnable(SKILL_TYPE.comet)`）。现在按住攻击键时被压掉的是
**突进冲击 / 凌空横斩 / 旋风斩击 / 彗星俯冲** 四个变招。
点按补发也按同一顺序补全：空中点按松手时 →
左右方向（+有落脚点）→ `WHEEL(_SHOTGUN)`；下方向（+有落脚点）→ `COMET(_SHOTGUN)`；
否则 → `AIRPUNCH(_SHOTGUN)`。

验证：`build=2026-09-26.4`，DLL SHA256 `BC3205EB712AE02F…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 56.6 【修复】旋风斩"全屏伤害"：半径多乘了 CLEN

**现象**：旋风斩一开，全屏敌人都在掉血。

**原因**：判定用的 `Physics2D.OverlapCircleAll` 半径误写成了 `半径 × mp.CLEN`。
这个工程的**物理世界单位与"格"是 1:1**（护符21 苦痛荆棘 `ThornsRadius`、
护符24 防御者纹章 `ShelterCircleRadius` 都是直接把"格数"当半径传给
`OverlapCircleAll`/`OverlapBoxAll`，挽歌剑气的判定箱同理），乘了 CLEN(=56) 之后
半径变成 100 多格 —— 等于全屏。

**改法**：`OverlapCircleAll(center, NailMasterCircleRadius, mask)`（去掉 ×CLEN，并在代码里写了注释）。
绿框渲染那边本来就是按 `半径 × 2 × CLEN` 换算成网格像素的（网格像素 ≠ 物理世界单位），
所以**框显示的大小一直是准的**——调配置时以绿框为准即可。

验证：`build=2026-09-26.5`，DLL SHA256 `202B275F25822852…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

## 57. 护符 36 编织者之歌（诺艾尔侧，2026-09-26，build=2026-09-26.6）

需求：**和小骑士那套做法一样**；并且**同时携带幼虫之歌（9）与编织者之歌（36）**时，
小蜘蛛的攻击**回复 3 魔力**。

### 57.1 移植自小骑士的那套（数值也照搬）

| 项 | 值（与小骑士一致） |
|---|---|
| 数量 / 过图重生 | 3 只；过图后等 **3 秒**再生成 |
| 跟随 | 贴地爬行，跟随速度 12 格/秒；诺艾尔在移动时追着她，停下时在 **1.2~2.6 格**内随机乱跑，偶尔跳（初速 -8、重力 28、时长 0.7s） |
| 离太远 | 超过 **8 格**（掉坑/被墙隔开）直接删除，下一帧重新生成 |
| 蛛丝 | 索敌 **7 格**、间隔 **2 秒**，速度 18 格/秒、存活 2.2 秒、碰撞半径 0.4 格；命中一次 **3 真伤**，命中即消失 |
| 近战 | 距离 ≤ **1.6 格** 且冷却（5 秒）到 → **15 真伤** |
| 贴地 | 用与骑士同一条路：`Map2d.BCC.isFallable(...)` 取脚下地面高度 |
| 素材/动画 | 直接读 `assets/hk/sheets/spider`（`spider_manifest.json` 的 Launch/Run/Idle/Attack + 近战 0014~0016 三帧），渲染缩放 0.28、朝向镜像、画在诺艾尔身后层 PR0 |

### 57.2 羁绊：幼虫之歌 + 编织者之歌 → 小蜘蛛命中回魔

- 判据：`IsEquipped(Noel, GrubsongId)`（幼虫之歌）；
- 触发点：蛛丝命中（每只敌人一次）与近战命中，各回 **`Charm36/GrubsongBondMp`（默认 3）** MP；
- 回魔走 `KnightInCradleBehaviour.GrantNoelMana(...)` + `RefreshNoelHudMp()`；
- 伤害本身是**真伤**（`fix_damage = true`），与小骑士的编织者/幼体一致；命中前照例跳过 `IsEnemySummoning`。

> 本版**未做**：小骑士那套"坐长椅时小蜘蛛睡觉/苏醒"的动画状态，以及"编织者+飞毛腿 → 攻击间隔减半"的羁绊。
> 需要的话我再补（结构已经按骑士那份留好了位置）。

验证：`build=2026-09-26.6`，DLL SHA256 `CCFA8538B4B42228…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 57.3 【修复】小蜘蛛生成在地里 + 渲染偏移/缩放做成配置

**现象**：小编织者一出现就陷在地面里。

**原因**：出生位置用了诺艾尔的**身体中心** `NoelBodyCenterY()` 当作基准，再让贴地逻辑只允许
"往更高处贴"（`targetY <= w.Y` 才吸附） → 站在地面附近时它们就停在身体中心那一层（比脚底低的那一侧），
看上去就是埋在土里；而且贴地查询的起点用的是"中心 + 0.75 格"，经常查不到脚下这块地面。

**改法**：
1. 出生位置改用**诺艾尔的脚底** `pr.mbottom - WeaverHalfH`，并向 `pr.mbottom` 查询地面；
   再加一条兜底：**任何情况下都不低于 `pr.mbottom - WeaverHalfH`**；
2. 贴地改成**双向吸附**（`|targetY - w.Y| <= 2` 就对齐），起点改成 `w.Y + WeaverHalfH`（脚底）；
3. 顺带把渲染的两个数做成配置，方便你按观感微调（默认值与小骑士那份一致）：
   `Charm36/RenderOffsetY`（默认 0.4 格，正值 = 上移）、`Charm36/RenderScale`（默认 0.28）。

验证：`build=2026-09-26.7`，DLL SHA256 `4A524002F86A1656…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

### 57.4 【修复】不跳跃 / 三只重合：上一版把骑士那套的两处逻辑改歪了

对照小骑士那份逐行核对，发现上一版（57.3）我自己"顺手简化"掉的两处正是问题根源：

| 现象 | 原因 | 修法（回到骑士那份的写法） |
|---|---|---|
| 不跳跃 | 57.3 我把贴地改成了"**双向吸附**"，而吸附里又顺手把 `HopT/HopVy` 清零 → **起跳那一帧就被按回地面**，等于没有跳跃 | 完全照抄骑士：跳跃条件是 `HopT <= 0 && NextHopT <= 0 && |HopVy| <= 0.01`；跳跃中**只在真正落到地面时**（`w.Y + HalfH >= gy`）才停；落地后走"脚下有地面就贴地、没地面就自然下坠"那条分支。顺带补上骑士的"诺艾尔站上高台时把小蜘蛛拉上台"逻辑 |
| 三只重合 | 上一版跟随目标写成了 `pr.x`（诺艾尔本人坐标）→ 三只都朝同一个点挤 | 恢复骑士的 `HomeOx`（出生时 `(idx-1)*1.3` 固定错开 -1.3 / 0 / +1.3），诺艾尔移动/离太远时目标都是 `pr.x + HomeOx` |

顺带补上骑士那份漏掉的**撞墙判据**：`NoelWeaverBlockCell()` 照抄 `IsBlockCell`
（越界算实心；`!CCON.isEmpty && !canStand && !isFloor && !isWater` 才算挡住），
所以小蜘蛛也不会再穿墙。

验证：`build=2026-09-26.8`，DLL SHA256 `8CB7E36D14BE6525…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

## 58. 护符 20 亡者之怒（诺艾尔侧，第一步：2026-09-26，build=2026-09-26.9）

需求（本批只做前两条，后续还有）：
1. **触发基准 30HP**：魔物攻击诺艾尔，若把她的 HP 打到 30 以下 → **立即恢复到 30HP** 并触发亡者之怒；
2. 触发瞬间：自动释放一次**不消耗魔力、不导致眩晕**的「圣光爆发」，并**清除身上所有负面效果**。

### 58.1 效果1：阈值触发

- 挂在已有的"诺艾尔受伤入口" `M2Attackable.applyHpDamage` 前缀（护符3/9/21 同一个挂点），
  前缀签名补上 `AttackInfo Atk` 以便判断来源：
  **只认魔物攻击**（`Atk as NelAttackInfo` 的 `Caster` / `AttackFrom` 是 `NelEnemy`），
  地图伤害/自伤不触发（与需求"魔物攻击诺艾尔"一致）；
- 判据：`当前HP - 本次伤害 < 阈值(默认30)` → 本次伤害 `val = 0`、HP 设为 `max(当前HP, 30)`、
  刷新 HUD，然后走触发流程；
- `NoelFuryActive`（供后续伤害加成用）= **HP ≤ 阈值**（回到阈值之上就自动结束），每帧在
  `TickNoelFuryCharm` 里维护；
- 与护符30 乔尼的祝福冲突：乔尼的 HP 不参与结算，所以**佩戴乔尼时本条不生效**（沿用"暂不处理"）。

### 58.2 效果2：自动圣光爆发 + 全清负面

- 全清：`pr.Ser.CureAll()`（AIC 自己的"全解"，把所有状态条目 deactivate）；
- 自动爆发：`pr.changeState(PR.STATE.BURST)`（圣光爆发的状态机自己会跑完动画/特效/伤害），
  同时打开一个 `AutoBurstSeconds`（默认 3 秒）的"免费窗口"：
  - **免魔**：前缀挂 `PR.applyBurstMpDamage`（爆发扣魔的入口）→ 窗口内直接跳过；
  - **免眩晕**：`M2Ser.Add` 前缀在窗口内拒绝 `BURST_TIRED / TIRED / OVERRUN_TIRED`；
- 已经处于 `STATE.BURST` 时不重复触发（避免连续受击把爆发刷成连播）。

配置（`[Charm20]`）：`HpThreshold`(30)、`AutoBurstSeconds`(3)。

验证：`build=2026-09-26.9`，DLL SHA256 `917096E374428618…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `71 成功 / 0 失败`）。

> **工程卫生提示（写给以后的自己）**：这次为了把误插到类外的代码块搬回类内，我用
> `[System.IO.File]::WriteAllLines` 整файл重写过 `CharmEffects.cs`，**行尾符被顺手改成了 CRLF**，
> 于是那一条提交看起来有 6000+ 行改动（其实是纯格式）。随后已用 `WriteAllText` + LF 重新写回，
> **现在整文件统一为 LF**（此前是 CRLF/LF 混用）。以后批量改这个文件**只用 `apply_patch`**，
> 或者写回时显式用 LF，避免再出现这种"整文件 diff"。
> 行尾恢复后重新编译/部署：`build=2026-09-26.9`，DLL SHA256 `5BBAE128B93F35AF…`（代码未变，只是重编）。

### 58.3 【修正】触发时 HP **直接设为 30**

需求澄清：触发亡者之怒时 HP 要"**直接设置为 30**"，而不是我最初写的
`max(当前HP, 30)`（那样 HP 高于 30 时等于完全免掉这次伤害、HP 不变）。
现在改成 `PrHpField.SetValue(noel, HpThreshold)` —— 无论触发前 HP 是多少，
触发后一律落到 **30HP**，同时 `NoelFuryActive`（HP ≤ 阈值）随之成立。

验证：`build=2026-09-26.10`，DLL SHA256 `ABFC18E3BF4AB299…`（两份安装已同步；只覆盖 DLL）。

### 58.4 【修正】只有"HP 降到 30"那一次才触发圣光爆发

需求澄清：**正好 30HP 时再被魔物攻击，不会重复触发圣光爆发**；
只有"HP 从别的值（高于 30）被打到 30"这一下才算触发事件。

改法：`TryTriggerNoelFury` 里区分两件事：

| 情况 | 伤害 | HP | 圣光爆发 + 全清负面 |
|---|---|---|---|
| `hp != 30` 且这一下会打到 30 以下 | 不结算 | 直接设为 30 | **触发** |
| `hp == 30` 且再被魔物攻击 | 不结算 | 保持 30 | 不触发（`_noelFuryActive` 保持为真） |

（`hp != 30` 覆盖了"高于 30 被打下来"与本就用别的手段掉到 30 以下后又被魔物打到 30"两种；
...
### 58.13 按 `docs/护符加成描述.md` 对齐加成范围 + 补齐羁绊（build=2026-09-26.20）

用户给出《护符加成描述.md》（技能 × 护符的 √ 表 + 羁绊清单），逐格核对后改了这些：

| 项 | 原状 | 改后（对齐表格） |
|---|---|---|
| 17 快速劈砍 / 20 攻速 的适用招式 | `IsNoelAttackState` 含 WHEEL / COMET / DASHPUNCH / SLIDING | 只留 轻攻击(PUNCH) / 凌空横斩(AIRPUNCH[+SG]) / 会心重击(SMASH[+SG]) / 轮舞斩击(EVADECOUNTER[+SG]) —— 表里旋风斩击/彗星俯冲没勾 |
| 18 修长之钉 / 19 骄傲印记 的适用招式 | `IsLongNailKind` = PR_PUNCH + PR_SHOTGUN + PR_SMASH | 去掉 PR_SMASH（表里会心重击的 18/19 栏为空） |
| 21 苦痛荆棘 的反击伤害 | 只吃 `ThornsDamageMult` | 再加 13 坚固力量 / 16 会心 / 20 亡者之怒（不吃萨满） |
| 23 吸虫伤害 | 固定 7 / 带萨满 9 | 再乘 16 会心 / 20 亡者之怒（不吃 13、不吃萨满的 1.25） |
| 24 防御者纹章法阵伤害 | 固定 10 | 再乘 5 萨满 / 16 会心 / 20 亡者之怒（不吃 13） |
| 10 附魔剑气命中 | 不回魔 | 补上 4 灵魂捕手(+6) / 6 噬魂者(+15)（未附魔剑气仍不回魔） |

为此新增了一个**辅助伤害源乘区** `NoelSideDamageMult(bool powerKind)`：
只含 13（可选）/16/20，**不**含护符35 的 ×5 与护符27 的"深聚下一击"，
免得辅助伤害把深聚的"下一击"额度吃掉。
...
| 羁绊 | 状态 |
|---|---|
| 3+30 单次≤50 + 受伤 2 秒无敌 | 已有 |
| 5+23 吸虫 7→9 | 已有 |
| 7+33 取消锋利之影冲刺前摇 | 已有 |
| **8+36 小编织者攻速加快** | 新增：`[Charm36] RunnerBondCooldownScale`（默认 0.75 = 攻速约 +33%，给攻击/近战冷却乘缩放） |
| 9+36 小编织者命中回 3 魔力 | 已有 |
| **14+33 冲刺耗魔 70→60** | 新增：`Charm33/DashMpCost` 默认 100 → **70**（两份 cfg 里已有的值也已手工改成 70），佩戴 14 时用 `Charm33/TwistedBondMpCost`（默认 60） |
| 18+19 距离叠加 | 已有（百分比相加） |
| **26+34 蹲下/爬行按同速率回魔** | 新增：乌恩之形回血那份里并行走一个独立累加器（HP 满时也继续回魔），速率 = `Charm34/CrouchHealPerSecond` |
| **28+29+30 额外 70 魔力上限** | 数值 90 → **70**（改为 `[Charm30] BlueHeartTripleMpBonus`，默认 70） |

**已确认**：`14 法术扭曲者` 减免**魔法霰弹的蓄力耗魔**（挂 `M2PrSkill.digestShotgunHoldMp`）
是要保留的行为 —— 用户已在 `docs/护符加成描述.md` 的"魔法霰弹 / 附魔X"行把 14 栏补成 √，
代码无需改动（提交 `651ffb5`）。

验证：`build=2026-09-26.20`，DLL SHA256 `4AAA9B84A8929363…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `0 失败`，`[Charm30] BlueHeartTripleMpBonus` / `[Charm33] TwistedBondMpCost` /
`[Charm36] RunnerBondCooldownScale` 三个新配置键已生成）。

## 59. 诺艾尔护符的"未解锁 / 解锁"（第一步：界面文本，build=2026-09-26.21）

需求来源：`docs/护符效果描述.md`。文档除了文案以外还规定了机制（**初始 3 个护符槽**、
所有护符默认锁定、**每开 4 个宝箱 +1 槽（最多 +8）**、**炼金最多再 +3 槽**、
37〜40 可选中但无法佩戴）—— 那些留到下一步，这一步只落**界面文本**。

### 59.1 数据侧（`src/CharmUi/CharmData.cs`）

`CharmData` 拆成两段文案，和文档的"未解锁 / 解锁"两行一一对应：

| 字段 | 对应文档 | 用途 |
|---|---|---|
| `LockedText` | `未解锁：…` 那一行 | 解锁条件的提示 |
| `Desc` | `解锁：…` 那一行（含 `\n\n` 后的风味文本） | 已解锁时的说明 |

同时按文档把 **费用** 全部改掉（例：3 坚硬外壳 2→3、5 萨满之石 3→4、10 蜕变挽歌 3→4、
13 坚固力量 3→4、14 法术扭曲者 2→4、20 亡者之怒 2→4、33 锋利之影 2→5、35 骨钉大师 1→5）。
后续两次按用户口头修正：`4 灵魂捕手 = 2`、`6 噬魂者 = 4`、`14 法术扭曲者 = 3`、`33 锋利之影 = 4`
（`build=2026-09-26.24`）——最终以代码里的数值为准。

⚠ **编号对齐说明**：文档末尾写的是 `40.国王之魂`，而工程里的编号是
`40 = 虚空之心（固定护符，不可卸下）`、`41 = 国王之魂` —— 文档没有虚空之心的条目。
所以这次是**按名字**对齐：舞梦者→37、梦之盾→38、格林之子→39、国王之魂→**41**，
虚空之心（40）保持原样。若你想把两者的编号也对调成文档那样，说一声即可。

### 59.2 界面侧（`src/CharmUi/CharmUiOnGuiLayer.cs`）

详情面板的说明行现在按状态显示（**不带"未解锁：/解锁："前缀** —— 面板本身就是状态，
前缀属于文档里标注哪一行用哪段文案的写法，2026-09-26.22 起已去掉）：

| 状态 | 显示 |
|---|---|
| 未解锁 | `<LockedText>`（解锁条件那行） |
| 已解锁 | `<Desc>`（效果说明 + `\n\n` 后的风味文本） |
| 虚空之心 | `这个护符是持有者的一部分，不能卸下。`（原样） |

名称与 `花费：N` 仍是**两个独立控件**（文档把费用画在名称后面的括号里，但现有面板是
单独一行的"花费："；没改布局，避免动 UI JSON 的位置。要合并成 `名称（N）` 随时可以改）。

### 59.3 锁定状态从哪来（暂时的）

真正的解锁系统还没做，所以"是否未解锁"现在由 `CharmDatabase.IsLocked(id)` 提供，
默认**全部视为已解锁**（= 现有行为不变），只留一个调试开关：
`[CharmUi] PreviewLockedText`（默认 false）—— 打开后所有护符都显示"未解锁：…"，
方便你在游戏里逐条核对文案。等解锁机制落地时，只把 `IsLocked` 改成读存档里的解锁集合即可，
UI 侧不用再动。

验证：`build=2026-09-26.21`，DLL SHA256 `1B31DB8A5AB08B5D…`（两份安装已同步；只覆盖 DLL；
本地隐藏启动确认 `0 失败`）。
本地隐藏启动确认 `71 成功 / 0 失败`）。
