...
## 28. 诺艾尔的护符第二部分（10）：沉重之击（id 16，2026-09-22，build=2026-09-22.49）

需求：诺艾尔造成伤害时，**20% 概率让这一发伤害翻倍**。

实现：直接并入已有的"诺艾尔侧最终伤害乘区"挂点（`MGContainer.CircleCast` 前缀/后缀，
与护符 5 萨满之石、护符 13 坚固力量共用同一个 `__state` 保存/还原），因此三种护符**连乘**且互不污染：

```
Atk.hpdmg0(临时) = hpdmg0 × [萨满 1.25（法术）] × [坚固力量 1.25（骨钉系招式）] × [沉重之击 2（20% 骰子）]
```

| 项目 | 说明 |
|---|---|
| 覆盖范围 | 诺艾尔**造成的任何伤害**——所有走 `CircleCast` 的攻击（轻攻击/各骨钉技艺/法术/魔法霰弹），不限定招式 |
| 骰子 | `UnityEngine.Random.value < 0.2f`（与模组其它概率实现一致），**每发攻击掷一次**，同一次 `CircleCast` 命中的多个目标一起翻倍 |
| 生效条件 | 本地诺艾尔模式（`!IsKnightMode`）+ `Mg.Caster is PRNoel` + 装备沉重之击；小骑士模式的伤害不参与 |
| 蜕变挽歌剑气 | 剑气伤害由模组自己结算（不走 `CircleCast`），已单独在这条路上也加了同样的 20% 翻倍（`ApplyNoelElegyDamage`），18 → 36 |
...
验证：`build=2026-09-22.49`，DLL SHA256 `A0CA254D157088EF…`（两份安装已同步；只覆盖 DLL）。
